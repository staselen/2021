            ...
            while (true)
            {
                lock (splitBuffer)
                {
                    ...
                    {   //If it's a soda, lock the sodaBuffer
                        lock (sodaBuffer)
                        {   
                            Console.WriteLine("{0}: Moved a {1} to the {1} Buffer", Thread.CurrentThread.Name, splitBuffer.Peek().Name);
                    ...
                    else if (splitBuffer.Peek().Name == "Beer")
                    {
                        lock (beerBuffer)
                        {
                            Console.WriteLine("{0}: Moved a {1} to the {1}", Thread.CurrentThread.Name, splitBuffer.Peek().Name);
