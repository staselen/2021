    ...
    class Philosopher
    {
        ...
        public void Dining(object forksConverted)
        {
            ...
            while (Plate.Food.Count != 0)
            {
                ...
                Monitor.Enter(forks[Math.Min(Position, (Position + 1) % 5)]);
                try
                {
                    ...
                    Monitor.Enter(forks[Math.Max(Position, (Position + 1) % 5)]);
                    try
                    {
                        Thread.Sleep(200);
                    ...
                    {
                        Monitor.PulseAll(forks[Math.Max(Position, (Position + 1) % 5)]); //Notifies the lock(Fork) is relased
                        Monitor.Exit(forks[Math.Max(Position, (Position + 1) % 5)]);     //Releases the lock(Fork)
                    }
                    Thread.Sleep(500);
                ...
                {
                    Monitor.PulseAll(forks[Math.Min(Position, (Position + 1) % 5)]); //Release the lock(Fork)
                    Monitor.Exit(forks[Math.Min(Position, (Position + 1) % 5)]);  //Notifies the lock(Fork) is released
